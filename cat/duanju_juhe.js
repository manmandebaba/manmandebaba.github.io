...
// 分类排除规则
const cate_remove = ['分类排除', '软鸭','碎片', '锦鲤', '番茄', '甜圈','牛牛']; 
...
async function request(url, options = {}) {
    let reqHeaders = { ...aggConfig.headers.default, ...options.headers };
    
    // 判断是否直接发送原始数据
    const isRawData = reqHeaders['Content-Type'] === 'text';
    
    let postType = isRawData ? '' : (
        reqHeaders['Content-Type']?.includes('json') ? 'json' :
...
async function init(cfg) {
  ...
  try {
    ...
    const res = safeJSONParse(response);
  ...
  try {
    ...
    let tkRes = safeJSONParse(tkhtml);
    ...
    let accesstoken = safeJSONParse(logindata);
...
async function category(tid, pg, filter, extend) {
  ...
  try {
    switch (tid) {
      ...
      case '甜圈': {
        const url = `${platRule.host}${platRule.list}=${encodeURIComponent(area)}&offset=${page}`;
      ...
      case '番茄': {
        ...
        let url = `${platRule.host}${platRule.list}?change_type=0&selected_items=${encodeURIComponent(area)}&tab_type=8&cell_id=6952850996422770718&version_tag=video_feed_refactor&device_id=1423244030195267&aid=1967&app_name=novelapp&ssmix=a&session_id=${sessionId}`;
      ...
      case '七猫': {
        ...
        const url = `${platRule.host}${platRule.list}?tag_id=${encodeURIComponent(area)}&playlet_privacy=1&operation=1&sign=${sign}`;
      ...
      case '河马': {
        try {
          ...
          if (dehtml) {
            ...
            if (hmdata && hmdata !== '{}') {
              const hmlist = safeJSONParse(hmdata).columnData || [];
    ...
    console.log(`【${siteName}】category失败: ${tid} - ${e.message}`);
  }
  ...
  try {
    ...
    switch (platform) {
      ...
      case '牛牛': {
        ...
        const descRes = safeJSONParse(descData);
        ...
        const listRes = safeJSONParse(listData);
      ...
      case '河马': {
        ...
        const detailRes = safeJSONParse(detailResponse);
        const detailHtml = detailRes.data;
        const postdata = hemaDecrypt(detailHtml);
        const videoInfo = safeJSONParse(postdata).videoInfo || {};
        ...
        const episodeRes = safeJSONParse(episodeResponse);
        const episodeHtml = episodeRes.data;
        const playdata = hemaDecrypt(episodeHtml);
        const chapterList = safeJSONParse(playdata).chapterList || [];
    ...
    console.log(`【${siteName}】detail失败: ${e.message}`);
    return JSON.stringify({ list: [] });
  }
}
...
async function play(flag, id, flags) {
  try {
    if (/百度/.test(flag)) {
        const postData = { vid: id, method: "post" };
      ...
      if (!json?.clarityUrl) {
        return JSON.stringify({ parse: 0, url: id, msg: '获取播放链接失败' });
      }
    ...
    if (/番茄/.test(flag)) {
      ...
      if (res.data?.[id]) {
        ...
        return JSON.stringify({ parse: 0, url: url || id });
      }
      return JSON.stringify({ parse: 0, url: id });
    }
    ...
    if (/牛牛/.test(flag)) {
      ...
      if (inputArr.length === 2) {
        ...
        var result = safeJSONParse(response);
    ...
    if (/围观/.test(flag)) {
      try {
        let playSetting = typeof id === 'string' ? safeJSONParse(id, {}) : id;
        let urls = [];
        ...
        if (playSetting['1080P']) urls.push("1080P", playSetting['1080P']);
        if (playSetting['720P']) urls.push("720P", playSetting['720P']);
        if (playSetting['480P']) urls.push("480P", playSetting['480P']);
        if (urls.length) {
    ...
    if (/河马/.test(flag)) {
      try {
        ...
        let res = safeJSONParse(response);
        let fshtml = res.data;
        if (fshtml) {
          let fsdata = hemaDecrypt(fshtml);
          if (fsdata && fsdata !== '{}') {
            let parsed = safeJSONParse(fsdata);
    ...
    console.log(`【${siteName}】play失败: ${e.message}`);
...
async function cfs(siteId, wd, pg) {
  ...
  try {
      ...
      case '河马': {
        try {
          ...
          if (xmres) {
            ...
            if (dexmres && dexmres !== '{}') {
              const xmlist = safeJSONParse(dexmres).searchVos || [];
    ...
    console.log(`【${siteName}】cfs失败: ${siteId} - ${e.message}`);
  }
...
async function search(wd, quick, pg) {
  ...
  const searchPromises = platForms.map(async (platform) => {
    ...
    return safeJSONParse(result).list || [];
  });
...
async function getSuiPianToken() {
  try {
    ...
    const tokenRes = safeJSONParse(tokenResponse);
    return tokenRes.data?.token || '';
  } catch (e) {
    console.log(`获取碎片token失败: ${e.message}`);
    return '';
  }
}
...
async function niuniuPost(url1, data1, index) {
  try {
    ...
    return safeJSONParse(html1);
  } catch (e) {
    console.log(`niuniuPost失败: ${e.message}`);
    return {};
  }
}
